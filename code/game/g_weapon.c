...
// Bryar Pistol
//--------
#define BRYAR_PISTOL_VEL			1600 * jp_projectileVelocityScale.value
#define BRYAR_PISTOL_DAMAGE			10 * jp_weaponDamageScale.value
...
#define BLASTER_VELOCITY			2300 * jp_projectileVelocityScale.value
#define BLASTER_DAMAGE				20 * jp_weaponDamageScale.value

// Tenloss Disruptor
//----------
#define DISRUPTOR_MAIN_DAMAGE			30 * jp_weaponDamageScale.value//40
#define DISRUPTOR_NPC_MAIN_DAMAGE_CUT	0.25f

#define DISRUPTOR_ALT_DAMAGE			100 * jp_weaponDamageScale.value//125
...
// Wookiee Bowcaster
//----------
#define	BOWCASTER_DAMAGE			50 * jp_weaponDamageScale.value
#define	BOWCASTER_VELOCITY			1300 * jp_projectileVelocityScale.value
...
#define	REPEATER_DAMAGE				14 * jp_weaponDamageScale.value
#define	REPEATER_VELOCITY			1600 * jp_projectileVelocityScale.value

#define REPEATER_ALT_SIZE				3	// half of bbox size
#define	REPEATER_ALT_DAMAGE				60 * jp_weaponDamageScale.value
#define REPEATER_ALT_SPLASH_DAMAGE		60 * jp_weaponDamageScale.value
#define REPEATER_ALT_SPLASH_RADIUS		128
#define	REPEATER_ALT_VELOCITY			1100 * jp_projectileVelocityScale.value

// DEMP2
//----------
#define	DEMP2_DAMAGE				35 * jp_weaponDamageScale.value
#define	DEMP2_VELOCITY				1800 * jp_projectileVelocityScale.value
#define	DEMP2_SIZE					2		// half of bbox size

#define DEMP2_ALT_DAMAGE			8 * jp_weaponDamageScale.value//12		// does 12, 36, 84 at each of the 3 charge levels.
...
#define FLECHETTE_DAMAGE			12 * jp_weaponDamageScale.value//15
#define FLECHETTE_VEL				3500 * jp_projectileVelocityScale.value
#define FLECHETTE_SIZE				1
#define FLECHETTE_MINE_RADIUS_CHECK	256
#define FLECHETTE_ALT_SHOTS			2
#define FLECHETTE_ALT_DAMAGE		60 * jp_weaponDamageScale.value
#define FLECHETTE_ALT_SPLASH_DAM	60 * jp_weaponDamageScale.value
#define FLECHETTE_ALT_SPLASH_RAD	128

// Personal Rocket Launcher
//---------
#define	ROCKET_VELOCITY				900 * jp_projectileVelocityScale.value
#define	ROCKET_DAMAGE				100 * jp_weaponDamageScale.value
#define	ROCKET_SPLASH_DAMAGE		100 * jp_weaponDamageScale.value
...
// Stun Baton
//--------------
#define STUN_BATON_DAMAGE			20 * jp_weaponDamageScale.value
#define STUN_BATON_ALT_DAMAGE		20 * jp_weaponDamageScale.value
...
//		missile->count = count;		// The single player stores the charge in count, which isn't accessible on the client
	...
	if (jp_tweakWeapons.integer & WT_PROJECTILE_GRAVITY) //JAPRO - Serverside - Give bullets gravity!
...
//---------------------------------------------------------
{
	...
	if ( altFire )
	{
		...
		else if (jp_tweakWeapons.integer & WT_PSEUDORANDOM_FIRE)
		{
...
//---------------------------------------------------------
{
	...
	if (jp_tweakWeapons.integer & WT_DISRUPTOR_DAM)
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < count; i++ )
	{
		...
		// add some slop to the alt-fire direction
		if (jp_tweakWeapons.integer & WT_BOWCASTER_SPRD) {
			if (i == 1) {
			...
			if (jp_tweakWeapons.integer & WT_PSEUDORANDOM_FIRE) {
...
//	missile->mass = 10;		// NOTENOTE No mass yet
	...
	if (jp_tweakWeapons.integer & WT_REPEATER_ALT_DAM) {
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < FLECHETTE_SHOTS; i++ )
	{
		...
		if (!(jp_tweakWeapons.integer & WT_FLECHETTE_SPRD)) {
			if (i != 0)
...
//------------------------------------------------------------------------------
{
	...
	if (jp_tweakWeapons.integer & WT_FLECHETTE_ALT_SPRD)
		missile = CreateMissileNew(start, fwd, ((1000 + 100 * (i)) * jp_projectileVelocityScale.value), 1500 + random() * 2000, self, qtrue, qtrue, qtrue); //mean of 1050
	else
		missile = CreateMissileNew(start, fwd, 700 + random() * 700, 1500 + random() * 2000, self, qtrue, qtrue, qtrue);
	...
	if (jp_tweakWeapons.integer & WT_FLECHETTE_ALT_DAM) {
...
//---------------------------------------------------------
{
	int			damage = 4 * jp_weaponDamageScale.value; // LG nerf even though it didnt nead it vs good players.. woop
	qboolean	render_impact = qtrue;
	vec3_t		start, end;
	trace_t		tr;
	gentity_t	*traceEnt, *tent;
	...
	VectorMA(start, 1, right, start);

	if (jp_unlagged.integer & UNLAGGED_HITSCAN)
	...
	if (render_impact)
	{
		if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage)
		{
			...
			if ((jp_tweakWeapons.integer & WT_STUN_HEAL) && !ent->client->ps.duelInProgress)
...
//---------------------------------------------------------
{
	int			damage = 55 * jp_weaponDamageScale.value;
	qboolean	render_impact = qtrue;
	vec3_t		start, end;
	trace_t		tr;
	gentity_t	*traceEnt, *tent;
	...
	if (render_impact)
	{
		if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage)
		{
			...
			G_Damage(traceEnt, ent, ent, forward, tr.endpos, damage, DAMAGE_NORMAL, MOD_UNKNOWN); //I guess keep this as turblast since MOD_STUN is used for healgun .. and we dont want shocklance to heal :\
...
void CalcMuzzlePoint ( gentity_t *ent, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) 
{
	...
	{	// Use the table to generate the muzzlepoint;
		if (jp_tweakWeapons.integer & WT_CENTER_MUZZLEPOINT && (ent->client->pers.centerMuzzle || ent->client->sess.raceMode)) //Also centerMuzzle for racemode rocketjump styles
		{
			switch (weapontype)
			{
			case WP_STUN_BATON:
...
void FireWeapon( gentity_t *ent, qboolean altFire ) {
	...
	if (ent->s.weapon != WP_SABER && (ent->s.weapon != WP_STUN_BATON || ((jp_tweakWeapons.integer && WT_STUN_LG) || (jp_tweakWeapons.integer && WT_STUN_SHOCKLANCE))))
	{
...
//	CalcMuzzlePointOrigin ( ent, ent->client->oldOrigin, forward, right, up, muzzle );
	...
	switch( ent->s.weapon ) {
	case WP_STUN_BATON:
		if (jp_tweakWeapons.integer & WT_STUN_LG && !altFire)//JAPRO - Lightning Gun
			WP_FireLightningGun(ent);
		else if (jp_tweakWeapons.integer & WT_STUN_SHOCKLANCE && altFire)//JAPRO - Shocklance
